...
for batch_size in batch_sizes:
    ...
    for _ in range(10):  # 训练10次取平均
        # 定义EarlyStopping和ModelCheckpoint回调函数
        early_stopping = EarlyStopping(monitor='val_loss', patience=5, verbose=1, restore_best_weights=True)
        model_checkpoint = ModelCheckpoint('best_model.h5', monitor='val_loss', save_best_only=True, verbose=1)

        # 训练神经网络模型
        history = model.fit(X_train, Y_train_nn, batch_size=batch_size, epochs=160, verbose=0,
        ...
        nn_accuracy = (tn + tp) / (tn + fp + fn + tp)

        batch_loss_values.append(history.history['loss'][-1])
        batch_accuracy_values.append(nn_accuracy)
    
    loss_values.append(np.mean(batch_loss_values))
    accuracy_values.append(np.mean(batch_accuracy_values))

# 绘制损失函数随 batch_size 变化的柱状图
plt.bar(range(len(batch_sizes)), loss_values)
plt.xlabel('Batch Size')
plt.ylabel('Loss')
plt.title('Average Loss vs. Batch Size')
...
# 绘制准确率随 batch_size 变化的柱状图
plt.bar(range(len(batch_sizes)), accuracy_values)
plt.xlabel('Batch Size')
plt.ylabel('Accuracy')
