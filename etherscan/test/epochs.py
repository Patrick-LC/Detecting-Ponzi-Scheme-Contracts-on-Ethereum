...
epochs_list = range(10, 501, 5)  # epochs范围从10到500，步长为5
loss_values = []
accuracy_values = []
...
for epochs in epochs_list:
    ...
    # 训练神经网络模型
    history = model.fit(X_train, Y_train_nn, batch_size=64, epochs=epochs, verbose=0,
    ...
    accuracy_values.append(nn_accuracy)

# 绘制损失函数随 epochs 变化的点图
plt.scatter(epochs_list, loss_values, marker='o')
plt.xlabel('Epochs')
plt.ylabel('Loss')
...
# 绘制准确率随 epochs 变化的点图
plt.scatter(epochs_list, accuracy_values, marker='o')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
