...
# 定义隐藏层数量的范围
hidden_layer_nums = [1, 2, 3, 4, 5]

# 初始化结果列表
losses = []
accuracies = []

for num_layers in hidden_layer_nums:
    # 神经网络分类器
    model = Sequential()
    ...
    print("Neural Network Classifier F1 Score ({} hidden layers):".format(num_layers), nn_f1_score)

    # 根据混淆矩阵计算精度
    tn, fp, fn, tp = confusion_matrix(Y_test_nn, nn_pred).ravel()
    nn_accuracy = (tn + tp + fn) / (tn + fp + tp)
    print("Neural Network Classifier Accuracy ({} hidden layers):".format(num_layers), nn_accuracy)
...
plt.bar(hidden_layer_nums, losses)
plt.xlabel('Hidden Layer Number')
plt.ylabel('Loss')
plt.title('Loss vs. Hidden Layer Number')
plt.show()

# 绘制准确率-隐藏层数量柱状图
plt.figure(figsize=(10, 6))
plt.bar(hidden_layer_nums, accuracies)
plt.xlabel('Hidden Layer Number')
plt.ylabel('Accuracy')
